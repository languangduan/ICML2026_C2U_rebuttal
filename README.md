...
**Figure 3.(main paper)** Suppression--utility trade-off across datasets. Each row shows the sensitivity of **suppression** (left, blue) and **utility retention** (right, red) to steering strength (**$\lambda$**) and sparsity (**$K$**). Darker blue indicates stronger suppression, and lighter red indicates better utility retention (lower non-target degradation). The heatmaps show that moderate $\lambda$ values often provide the best balance.
